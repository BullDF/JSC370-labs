...
_Answer here._ There are `r counts` sars-cov-2 papers.
...
abstracts <- str_remove_all(abstracts, "<CopyrightInformation>(.|\\n)+|<[^>]+>")
abstracts <- str_replace_all(abstracts, "[\\s]+", " ")
abstracts <- str_remove_all(abstracts, "^ ")
abstracts <- str_remove_all(abstracts, " $")
abstracts_na <- which(is.na(abstracts)) |> length()
abstracts_na
```

  - How many of these don't have an abstract? 

_Answer here._ `r abstracts_na` of them don't have an abstract.
...
title_na <- which(is.na(titles)) |> length()
title_na
```

- How many of these don't have a title ?

_Answer here._ `r title_na` of them don't have a title.
