...
acc <- (num_correct) / nrow(heart_pred)
acc
```
...
plot(heart_boost$train.error, ylab = "error", type = "l", col = "red", lwd = 3)
lines(heart_boost$cv.error, col = "blue", lwd = 3)

summary.gbm(heart_boost)

plot(heart_boost, i.var = c(5, 1, 8))
```
...
plot(heart_boost$cv.error)

yhat_boost <- predict(heart_boost, heart_test) # predict on test
...
plot(heart_boost1$cv.error)

yhat_boost1 <- predict(heart_boost1, heart_test) # predict on test

# calculate MSE
mean((yhat_boost1 - heart_test$AHD) ^ 2)
...
plot(heart_boost2$cv.error)

yhat_boost2 <- predict(heart_boost2, heart_test) # predict on test
...
plot(heart_boost3$cv.error)

yhat_boost3 <- predict(heart_boost3, heart_test) # predict on test
...
tune_grid <- expand.grid(
  max_depth = c(1, 3, 5, 7), # larger makes model more complex and potentially overfit
...
output <- capture.output(heart_xgb <- caret::train(
  as.factor(AHD) ~ .,
  heart_train,
  na.action = na.omit,
  method = "xgbTree",
...
# Predict
xgb_pred <- predict(heart_xgb, heart_test)

# Compute MSE
mean((as.numeric(xgb_pred) - as.numeric(heart_test$AHD)) ^ 2)
```

- Compare the the performance of the different models and summarize

We constructed four gradient boosting models. Among the four, the xgboost model achieves the lowest MSE of 1.297. The worst model was trained using a shrinkage of 0.01 and an interaction depth of 2.
